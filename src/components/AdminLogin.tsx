...
export default function AdminLogin({
  ...
  const handleRegisterDirectly = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (existingEmail) {
        setErrorMessage("Email này đã được sử dụng bởi tài khoản khác!");
      ...
      // Hash the password for local encryption
      const hashedPass = await hashPassword(regPassword);
  ...
  const handleLocalLoginSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 1. Try local login (admin legacy password stored in DB)
      ...
      // 2. Hash password and search normal user accounts
      const hashedPass = await hashPassword(loginPassword.trim());
      const user = await dbService.loginUser(loginKey.trim(), hashedPass);
      if (user) {
        onLoginSuccess(user);
        if (window.showNotification) {
          window.showNotification(
