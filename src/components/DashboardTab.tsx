      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 sm:gap-5">
        {metrics.map((card, i) => (
          ...
          >
            ...
            <p className="text-xl sm:text-2xl font-bold text-slate-850 mt-1 font-mono tracking-tight">
              {card.value}
            </p>
          </div>
        ))}
          ...
          <table className="w-full text-left border-collapse min-w-[500px]">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-100">
                <th className="px-4 sm:px-6 py-4.5 text-[10px] font-bold text-slate-400 uppercase tracking-widest font-mono">
                  Nguồn
                </th>
                <th className="px-4 sm:px-6 py-4.5 text-[10px] font-bold text-slate-400 uppercase tracking-widest font-mono">
                  Đường dẫn đích
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredTableData.map((row) => (
                ...
                >
                  ...
                  <td className="px-4 sm:px-6 py-4 font-mono text-xs text-indigo-600 font-semibold break-all max-w-[200px]">
                    {row.destUrl}
                  </td>
