...
import { DBChampion, DBItem, DBSpell, DBBadge, DBBuildGuide } from '../supabase';
...
const matchChampLane = (champ: DBChampion, lane: string): boolean => {
  ...
  if (lane === 'Caesar') {
    return role.includes('đấu sĩ') || role.includes('đỡ đòn') || role.includes('caesar');
  }
  if (lane === 'Rừng') {
    return role.includes('sát thủ') || role.includes('rừng') || name === 'florentino' || name === 'nakroth';
  }
  if (lane === 'Giữa') {
  ...
  if (lane === 'Trợ thủ') {
    return role.includes('trợ thủ') || role.includes('đỡ đòn') || role.includes('hỗ trợ');
  }
  return true;
};

const normalizeItemCategory = (loai: string | null): string => {
  if (!loai) return 'CONG';
  ...
  if (normalized === 'TOC_CHAY' || normalized === 'TỐC ĐỘ' || normalized === 'TOC_DO' || normalized === 'TỐC') return 'TOC_CHAY';
  if (normalized === 'RUNG' || normalized === 'ĐI RỪNG' || normalized === 'DI_RUNG') return 'RUNG';
  if (normalized === 'TRO_THU' || normalized === 'TRỢ THỦ' || normalized === 'TRỢ') return 'TRO_THU';
  return normalized;
};
...
export default function BuildGuidesTab({
  ...
  const [editingGuide, setEditingGuide] = useState<Partial<DBBuildGuide> | null>(null);
  ...
  // 6 Equipment Slots
  const [selectedItems, setSelectedItems] = useState<string[]>(Array(6).fill(''));
  ...
  const [rightTab, setRightTab] = useState<'champion' | 'item' | 'badge' | 'spell'>('champion');
  ...
  const activateSelector = (type: 'champion' | 'item' | 'spell' | 'badge', index?: number, key?: string) => {
  ...
  const handleSelectFromRightList = (id: string, name: string) => {
    if (activeSelector.type === 'champion') {
      ...
      const nextEmptyIdx = updated.findIndex((val, index) => index > idx && !val);
      if (nextEmptyIdx !== -1) {
        ...
        if (!selectedSpellId) {
          ...
          const firstEmptyBadgeKey = Object.keys(selectedBadges).find(k => !selectedBadges[k]);
      ...
      if (key) {
        ...
        const nextEmptyKey = badgeKeys.slice(currentKeyIdx + 1).find(k => !selectedBadges[k]);
        if (nextEmptyKey) {
  ...
  const handleClearSlot = (e: React.MouseEvent, type: 'champion' | 'item' | 'spell' | 'badge', index?: number, key?: string) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center bg-white p-5 rounded-2xl border border-slate-100 shadow-sm">
        <div>
          ...
          <p className="text-slate-400 text-xs mt-1">
            Quản lý các bộ khuyến nghị trang bị (1 tướng có thể có nhiều lối lên đồ khác nhau), bảng ngọc và phù hiệu.
          </p>
        </div>
        {!isEditing && (
          <button
            onClick={handleStartNew}
            className="px-4 py-2 text-xs font-bold text-white rounded-xl flex items-center gap-1.5 cursor-pointer shadow-md transition-all hover:brightness-115 active:scale-95"
      ...
      {errorMsg && (
        <div className="bg-rose-50 text-rose-700 border border-rose-100 p-4 rounded-xl text-xs flex items-center gap-2 font-semibold">
          <LucideIcon name="AlertTriangle" size={14} className="text-rose-500 shrink-0" />
          <span>{errorMsg}</span>
        </div>
      )}
      ...
      {isEditing ? (
        <form onSubmit={handleSave} className="space-y-6 animate-in fade-in duration-200">
          <div className="flex justify-between items-center bg-white p-4 rounded-2xl border border-slate-100 shadow-sm">
            <span className="font-bold text-sm text-slate-800 flex items-center gap-2">
              ...
              {editingGuide ? `Chỉnh sửa: ${editingGuide.tieu_de_giao_an || ''}` : 'Thiết lập Giáo án mới'}
            </span>
            <button
              type="button"
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
            ...
            <div className="lg:col-span-8 bg-white p-6 rounded-3xl border border-slate-100 shadow-sm space-y-6">
              ...
              <div className="space-y-1.5">
                <label className="block text-[10px] font-black text-slate-400 uppercase font-mono tracking-wider">Tiêu đề lối lên đồ *</label>
                <input
                  type="text"
                  value={title}
                  onChange={(e) => setTitle(e.target.value)}
                  placeholder="Ví dụ: Đấu Sĩ Solo Đường, Sát Thủ Rừng Siêu Chí Mạng, Full Chống Chịu..."
                  className="w-full bg-slate-50/50 border border-slate-250/70 rounded-xl px-4 py-2.5 text-xs focus:outline-none focus:border-indigo-500 text-slate-800 font-bold placeholder:font-normal placeholder:text-slate-400"
                  required
                />
              </div>
              ...
              <div className="space-y-2">
                <label className="block text-[10px] font-black text-slate-400 uppercase font-mono tracking-wider">Tướng đề xuất</label>
                <div 
                  onClick={() => activateSelector('champion')}
                  className={`relative border-2 rounded-2xl p-4 flex items-center gap-4 cursor-pointer transition-all ${
                ...
                >
                  {selectedChampId ? (
                    (() => {
                      const champ = champions.find(c => c.id === selectedChampId);
                      return (
                        <div className="flex items-center gap-4 w-full">
                          ...
                          <div className="flex-1 text-left">
                            ...
                            <p className="text-[10px] text-slate-400 mt-1">Đang chọn tướng này. Nhấn vào để đổi ở bảng bên phải.</p>
                          </div>
                          <button
                            type="button"
                    ...
                    <div className="flex flex-col items-center justify-center py-5 w-full text-center">
                      ...
                      <p className="text-xs font-bold text-slate-600">Chưa chọn Tướng đề xuất</p>
                      <p className="text-[10px] text-slate-400 mt-1">Chọn tướng từ danh mục bên phải để tiếp tục</p>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="space-y-3">
                <div className="flex justify-between items-center">
                  <label className="block text-[10px] font-black text-slate-400 uppercase font-mono tracking-wider">
                    Lối lên đồ (6 ô trang bị) *
                  </label>
                  <span className="text-[9.5px] text-slate-400 font-medium">Nhấp chọn ô để kích hoạt chọn nhanh</span>
                </div>
                <div className="grid grid-cols-3 sm:grid-cols-6 gap-3.5">
                  {Array.from({ length: 6 }).map((_, idx) => {
                    const itemId = selectedItems[idx];
                    const item = items.find(i => i.id === itemId);
                    const isActive = activeSelector.type === 'item' && activeSelector.index === idx;

                    return (
                      <div
                        key={idx}
                        onClick={() => activateSelector('item', idx)}
                        className={`relative aspect-square rounded-2xl border-2 flex flex-col items-center justify-center cursor-pointer transition-all p-2 ${
                          isActive
                            ? 'border-indigo-500 bg-indigo-50/20 ring-4 ring-indigo-100 shadow-md scale-102'
                            : 'border-slate-100 hover:border-slate-200 bg-slate-50/30'
                        }`}
                      >
                        <span className="absolute top-1.5 left-2 text-[8px] font-black text-slate-400 font-mono">
                          Ô {idx + 1}
                        </span>
                        ...
                        {item ? (
                          <div className="flex flex-col items-center justify-center w-full h-full mt-2">
                            <img 
                              ...
                              className="w-11 h-11 object-cover rounded-xl border border-slate-200 shadow-md"
                              referrerPolicy="no-referrer"
                            />
                          ...
                          <div className="flex flex-col items-center justify-center text-slate-400 mt-2">
                            ...
                            <span className="text-[8.5px] font-bold text-slate-400 mt-1">Nhấp chọn</span>
                          </div>
                        )}
                      </div>
                    );
              ...
              {/* Spell Section */}
              <div className="space-y-2">
                <label className="block text-[10px] font-black text-slate-400 uppercase font-mono tracking-wider">Phép phụ trợ</label>
                <div 
                  onClick={() => activateSelector('spell')}
                  className={`relative border-2 rounded-2xl p-3 flex items-center justify-between cursor-pointer transition-all max-w-sm ${
                ...
                >
                  {selectedSpellId ? (
                        ...
                        <div className="flex items-center justify-between w-full">
                          <div className="flex items-center gap-3">
                            ...
                            <div className="text-left">
                              <span className="font-extrabold text-xs text-slate-800">{spell?.ten_phu_tro}</span>
                              <p className="text-[9px] text-slate-400 mt-0.5">Chọn phép phụ trợ khuyên dùng.</p>
                            </div>
                          </div>
                          <button
                            type="button"
                    ...
                    <div className="flex items-center gap-2.5 text-slate-400 w-full justify-center py-1.5">
                      <LucideIcon name="Plus" size={14} />
                      <span className="text-xs font-bold text-slate-500">Chưa chọn phép phụ trợ</span>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="space-y-4 pt-2">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-[9.5px] text-slate-400 font-medium">Chọn lần lượt từng vị trí phù hiệu</span>
                </div>
                ...
                <div className="bg-slate-50/50 p-4 rounded-2xl border border-slate-100 space-y-3">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                    {['NHANH_CHINH_1', 'NHANH_CHINH_2', 'NHANH_CHINH_3', 'NHANH_CHINH_4'].map((key, idx) => {
                      const badgeId = selectedBadges[key];
                      const badge = badges.find(b => b.id === badgeId);
                      const isActive = activeSelector.type === 'badge' && activeSelector.key === key;

                      return (
                        <div
                          key={key}
                          onClick={() => activateSelector('badge', undefined, key)}
                          className={`relative p-3.5 rounded-xl border flex flex-col items-center justify-center text-center gap-1 cursor-pointer transition-all ${
                            isActive
                              ? 'border-indigo-500 bg-indigo-50/20 ring-4 ring-indigo-100 shadow-md scale-102'
                              : 'border-slate-200 bg-white hover:border-slate-300'
                          }`}
                        >
                          <span className="text-[8px] font-black text-slate-400 font-mono uppercase tracking-wider">
                            {idx === 0 ? 'Chọn Nhánh' : `Phù hiệu ${idx === 1 ? 'I' : idx === 2 ? 'II' : 'III'}`}
                          </span>
                          {badge ? (
                            <div className="flex flex-col items-center justify-center w-full mt-1.5">
                              ...
                              <button
                                type="button"
                                onClick={(e) => handleClearSlot(e, 'badge', undefined, key)}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {/* Nhánh Phụ 1 - 2 slots */}
                  <div className="bg-slate-50/50 p-4 rounded-2xl border border-slate-100 space-y-3">
                    <span className="text-[10px] font-black uppercase text-amber-600 font-mono tracking-wider flex items-center gap-1.5">
                  ...
                  {/* Nhánh Phụ 2 - 2 slots */}
                  <div className="bg-slate-50/50 p-4 rounded-2xl border border-slate-100 space-y-3">
                    <span className="text-[10px] font-black uppercase text-emerald-600 font-mono tracking-wider flex items-center gap-1.5">
              ...
              <div className="space-y-3 pt-2">
                <label className="block text-[10px] font-black text-slate-400 uppercase font-mono tracking-wider">Bảng Ngọc đề xuất (Nhập văn bản, nhấn Enter để xuống dòng)</label>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="space-y-1">
                    ...
                    <textarea
                      ...
                      className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-xs focus:outline-none focus:border-rose-500 text-slate-700 font-medium resize-none"
                    />
                  </div>
                  ...
                  <div className="space-y-1">
                    ...
                    <textarea
                      ...
                      placeholder="Ví dụ:&#10;Tốc đánh&#10;Tốc chạy"
                      className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-xs focus:outline-none focus:border-purple-500 text-slate-700 font-medium resize-none"
                    />
                  </div>
                  ...
                  <div className="space-y-1">
                    ...
                    <textarea
                      ...
                      placeholder="Ví dụ:&#10;Công phép&#10;Giảm hồi chiêu"
                      className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-xs focus:outline-none focus:border-emerald-500 text-slate-700 font-medium resize-none"
                    />
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2 pt-2">
                ...
                />
                <label htmlFor="g_active" className="text-xs font-bold text-slate-500 select-none cursor-pointer">
              ...
              <div className="flex justify-end gap-3 pt-4 border-t border-slate-100">
                <button
                  type="button"
                  onClick={() => setIsEditing(false)}
                  className="px-5 py-2.5 rounded-xl border border-slate-200 hover:bg-slate-50 text-xs font-extrabold text-slate-500 transition-colors cursor-pointer"
                >
                  Hủy bỏ
                </button>
                <button
                  type="submit"
                  disabled={loading}
                  className="px-6 py-2.5 rounded-xl text-xs font-extrabold text-white transition-all hover:brightness-110 active:scale-95 flex items-center gap-1.5 shadow-md cursor-pointer"
                  style={{ backgroundColor: accentColor }}
                >
                  {loading && <LucideIcon name="RefreshCw" size={12} className="animate-spin" />}
                  Lưu Giáo Án
                </button>
              </div>
            </div>
            ...
            <div className="lg:col-span-4 bg-white p-5 rounded-3xl border border-slate-100 shadow-sm space-y-4 lg:sticky lg:top-6">
              <div className="pb-2 border-b border-slate-100">
                <h3 className="font-black text-slate-800 text-xs uppercase tracking-wider flex items-center gap-2">
                  <LucideIcon name="Compass" className="text-indigo-500" size={14} />
                  Thư Viện Liên Quân
                </h3>
                <p className="text-[10px] text-slate-400 mt-1">Chọn từ danh sách dưới để gán nhanh vào vị trí đang active bên trái.</p>
              </div>
              ...
              <div className="relative">
                <input 
                  ...
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl pl-8 pr-4 py-2 text-xs focus:outline-none focus:border-indigo-500 text-slate-800"
                />
                <LucideIcon name="Search" className="absolute left-2.5 top-2.5 text-slate-400" size={13} />
                {searchTerm && (
                  <button 
                    type="button" 
              ...
              {rightTab === 'champion' && (
                <div className="space-y-3">
                  <div className="max-h-[350px] overflow-y-auto pr-1 space-y-1.5 scrollbar-thin">
                    {champions
                      .filter(c => {
                        const matchesSearch = c.ten_tuong.toLowerCase().includes(searchTerm.toLowerCase()) || 
                                              (c.vai_tro && c.vai_tro.toLowerCase().includes(searchTerm.toLowerCase()));
                        const matchesRole = champRole === 'ALL' || (c.vai_tro && c.vai_tro.includes(champRole));
                      ...
                      .map(c => {
                        ...
                        return (
                          <div
                            key={c.id}
                            onClick={() => handleSelectFromRightList(c.id, c.ten_tuong)}
                          ...
                          >
                            <img src={c.url_anh_dai_dien || ''} className="w-8 h-8 rounded-full object-cover border border-slate-200" alt="" referrerPolicy="no-referrer" />
                            <div className="flex-1 min-w-0 text-left">
                              <span className="block font-bold text-xs truncate">{c.ten_tuong}</span>
                              <span className="block text-[8.5px] text-slate-400 truncate">{c.vai_tro || 'Chưa rõ'}</span>
                            </div>
                            {isSelected && <LucideIcon name="Check" size={12} className="text-indigo-600 shrink-0" />}
                          </div>
                        );
                      })}
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-2 pt-1 border-t border-slate-50">
                    <div className="space-y-1">
                      <label className="text-[9px] font-bold text-slate-400 block">Vai trò</label>
                      <select
                        value={champRole}
                    ...
                    <div className="space-y-1">
                      <label className="text-[9px] font-bold text-slate-400 block">Đường đi (Lane)</label>
                      <select
                        value={champLane}
              ...
              {rightTab === 'item' && (
                <div className="space-y-3">
                  ...
                  <div className="max-h-[300px] overflow-y-auto pr-1 grid grid-cols-2 gap-2 scrollbar-thin">
                    {items
                      .filter(item => {
                        const matchesSearch = item.ten_trang_bi.toLowerCase().includes(searchTerm.toLowerCase());
                        const normalizedLoai = normalizeItemCategory(item.loai);
                        const matchesCat = itemCategory === 'ALL' || normalizedLoai === itemCategory;
                        return matchesSearch && matchesCat;
                      })
                      .map(item => {
                        const isEquippedInActive = activeSelector.type === 'item' && selectedItems[activeSelector.index ?? 0] === item.id;
                        const isAnywhere = selectedItems.includes(item.id);

                        return (
                          <div
                            key={item.id}
                            onClick={() => handleSelectFromRightList(item.id, item.ten_trang_bi)}
                            className={`p-2 rounded-xl border cursor-pointer transition-all flex flex-col items-center text-center gap-1.5 relative ${
                              isEquippedInActive
                            ...
                            title={item.mo_ta || ''}
                          >
                            <img src={item.url_hinh_anh} className="w-8 h-8 rounded-lg object-cover border border-slate-200" alt="" referrerPolicy="no-referrer" />
                            <span className="text-[8.5px] font-black text-slate-700 truncate w-full">{item.ten_trang_bi}</span>
                            {isEquippedInActive && (
              ...
              {rightTab === 'badge' && (() => {
                ...
                const isBranchSlot = activeKey === 'NHANH_CHINH_1' || activeKey === 'NHANH_PHU_1_1' || activeKey === 'NHANH_PHU_2_1';

                if (isBranchSlot) {
                  const branchBadges = badges.filter(b => branchRepresentativeIds.includes(b.id));
                  return (
                    <div className="space-y-3">
                      <div className="text-left py-1">
                        ...
                        <p className="text-[10px] text-slate-400 mt-0.5">Chọn 1 trong 4 phân nhánh phù hiệu chính dưới đây:</p>
                      </div>
                      <div className="grid grid-cols-1 gap-2 max-h-[350px] overflow-y-auto pr-1 scrollbar-thin">
                        {branchBadges.map(b => {
                          const isEquippedInActive = activeKey && selectedBadges[activeKey] === b.id;
                          return (
                            <div
                              key={b.id}
                              onClick={() => handleSelectFromRightList(b.id, b.ten_phu_hieu)}
                            ...
                            >
                              <img src={b.url_hinh_anh} className="w-9 h-9 rounded-full object-cover border border-slate-200 shadow-sm shrink-0" alt="" referrerPolicy="no-referrer" />
                              <div className="flex-1 min-w-0 text-left">
                                <span className="block font-black text-xs text-slate-800">{b.ten_phu_hieu}</span>
                                <span className="block text-[10px] text-slate-400 font-medium">Click để chọn nhánh này</span>
                              </div>
                              {isEquippedInActive && <LucideIcon name="Check" size={16} className="text-indigo-600 shrink-0" />}
                            </div>
                          );
                        })}
                      </div>
                ...
                if (!parentId) {
                  return (
                    <div className="p-8 text-center text-slate-500 text-xs font-semibold space-y-3 bg-slate-50 rounded-2xl border border-slate-100">
                      <LucideIcon name="AlertCircle" className="mx-auto text-amber-500" size={32} />
                      <p className="font-extrabold text-slate-700">Chưa chọn loại nhánh</p>
                      <p className="text-[10px] text-slate-400 font-medium">Vui lòng nhấp chọn ô "Chọn Nhánh" đầu tiên để thiết lập loại nhánh cho nhánh này trước.</p>
                    </div>
                  );
                }

                const parentBadge = badges.find(b => b.id === parentId);
                const parentBranch = parentBadge ? getBadgeBranch(parentBadge) : 'KHAC';
                ...
                const filteredBadges = badges.filter(b => {
                  ...
                  const matchesSearch = b.ten_phu_hieu.toLowerCase().includes(searchTerm.toLowerCase());
                  return isSameBranch && isNotRep && matchesSearch;
                });

                return (
                  <div className="space-y-3">
                    <div className="flex justify-between items-center bg-indigo-50/50 p-2 rounded-xl border border-indigo-100/40 text-left">
                      <div>
                        <span className="block text-[8px] font-black uppercase text-indigo-700 tracking-wider">Đang xem nhánh:</span>
                        <span className="block font-black text-[11px] text-slate-800">{branchLabelMap[parentBranch] || parentBranch}</span>
                      </div>
                    ...
                    <div className="max-h-[350px] overflow-y-auto pr-1 space-y-1.5 scrollbar-thin">
                      {filteredBadges.length === 0 ? (
                        <div className="p-6 text-center text-slate-400 text-xs">Không tìm thấy phù hiệu nào phù hợp.</div>
                      ) : (
                        filteredBadges.map(b => {
                          const isEquippedInActive = activeKey && selectedBadges[activeKey] === b.id;
                          const isAnywhere = Object.values(selectedBadges).includes(b.id);
                          ...
                          return (
                            ...
                            >
                              <img src={b.url_hinh_anh} className="w-7 h-7 rounded-full object-cover border border-slate-200 shadow-sm shrink-0" alt="" referrerPolicy="no-referrer" />
                              <div className="flex-1 min-w-0 text-left">
                                <span className="block font-bold text-xs truncate">{b.ten_phu_hieu}</span>
                              </div>
                              {isEquippedInActive && <LucideIcon name="Check" size={12} className="text-indigo-600 shrink-0" />}
                            </div>
                          );
                        })
                      )}
              ...
              {rightTab === 'spell' && (
                <div className="space-y-3">
                  <div className="max-h-[350px] overflow-y-auto pr-1 space-y-1.5 scrollbar-thin">
                    {spells
                      .filter(s => s.ten_phu_tro.toLowerCase().includes(searchTerm.toLowerCase()))
                      .map(s => {
                        const isSelected = selectedSpellId === s.id;
                        return (
                          <div
                            key={s.id}
                            onClick={() => handleSelectFromRightList(s.id, s.ten_phu_tro)}
                            ...
                            title={s.mo_ta || ''}
                          >
                            <img src={s.url_hinh_anh} className="w-7 h-7 rounded-full object-cover border border-slate-200 shadow-sm" alt="" referrerPolicy="no-referrer" />
                            <div className="flex-1 min-w-0 text-left">
                              <span className="block font-bold text-xs truncate">{s.ten_phu_tro}</span>
                            </div>
                            {isSelected && <LucideIcon name="Check" size={12} className="text-indigo-600 shrink-0" />}
                          </div>
                        );
        ...
        <div className="grid grid-cols-1 gap-4.5">
          {guides.map((g) => (
            <div key={g.id} className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm hover:shadow-md transition-shadow flex flex-col md:flex-row md:items-center justify-between gap-4 animate-in fade-in duration-200">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className="font-bold text-sm text-slate-800">{g.tuong?.ten_tuong || 'Tướng ẩn'}</span>
                    <span className="text-[10px] font-medium px-2 py-0.5 rounded-full bg-slate-100 text-slate-500">{g.tuong?.vai_tro}</span>
                    <span className={`text-[9px] font-bold px-2 py-0.5 rounded-full ${g.kich_hoat ? 'bg-emerald-50 text-emerald-600 border border-emerald-100' : 'bg-amber-50 text-amber-600 border border-amber-100'}`}>
                      {g.kich_hoat ? 'Công khai' : 'Ẩn'}
                    </span>
                  </div>
                  <h3 className="font-extrabold text-slate-700 text-xs mt-1.5 flex items-center gap-1">
                    <LucideIcon name="Sword" size={12} className="text-rose-500" />
                    {g.tieu_de_giao_an}
                  </h3>
                  ...
                  <div className="flex items-center gap-1.5 mt-2.5">
                    {g.trang_bi_list?.map((item, idx) => (
                      <div key={idx} className="relative group/item" title={item.ten_trang_bi}>
                        <img 
                          src={item.url_hinh_anh} 
              ...
              <div className="flex items-center gap-2.5 self-end md:self-auto border-t md:border-t-0 pt-3 md:pt-0 border-slate-50">
                <button
                  ...
                  className="p-2 bg-slate-50 hover:bg-slate-100 text-slate-500 hover:text-indigo-600 rounded-xl transition-all cursor-pointer flex items-center gap-1 text-[11px] font-bold"
                >
                ...
                <button
                  ...
                  className="p-2 bg-rose-50 hover:bg-rose-100 text-rose-500 hover:text-rose-700 rounded-xl transition-all cursor-pointer flex items-center gap-1 text-[11px] font-bold"
                >
          ...
          {guides.length === 0 && (
            <div className="text-center p-12 border border-dashed border-slate-200 rounded-2xl font-sans text-sm text-slate-400 bg-slate-50/50">
              Chưa có giáo án Liên Quân nào được tạo. Nhấp "Tạo Giáo Án Mới" để thiết lập bộ giáo án đầu tiên của bạn!
            </div>
          )}
        </div>
      )}
