  ...
  const handleCropperConfirm = async (croppedFile: File) => {
    ...
    try {
      ...
      if (publicUrlData.publicUrl) {
        ...
        window.showNotification?.(
          "Không tìm thấy URL ảnh sau khi tải lên!",
          "error",
        );
      }
    } catch (err: any) {
      ...
      window.showNotification?.(
        `Đã xảy ra lỗi khi tải ảnh lên: ${err.message || err}`,
        "error",
      );
    } finally {
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        {links.map((link, idx) => {
          ...
          return (
            <div
              key={link.id}
              className={`flex items-center gap-4 p-4 bg-white border border-slate-100 rounded transition-all duration-300 shadow-sm hover:shadow-md ${
                isDragged
            ...
            >
              ...
              <div className="flex items-center gap-3 shrink-0">
                ...
                <button
                  ...
                  style={{
                    backgroundColor: link.enabled ? accentColor : undefined,
                  }}
                >
                  <div
        ...
        {links.length === 0 && (
          <div className="bg-white border border-dashed border-slate-200 rounded-md p-12 text-center text-slate-400">
            <LucideIcon
              name="Globe"
              size={40}
            ...
            <p className="text-sm text-slate-400 mt-1 max-w-xs mx-auto">
              Thêm liên kết đầu tiên của bạn bằng cách nhấp vào nút phía trên để
              bắt đầu xây dựng trang tiểu sử của bạn.
            </p>
          </div>
        )}
      </div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 bg-slate-900/40 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-md rounded-md p-6 shadow-2xl border border-slate-100 transform transition-all animate-in fade-in zoom-in-95 duration-200">
            {/* Modal Header */}
            <div className="flex justify-between items-start mb-5">
              <div>
                ...
                <p className="text-slate-400 text-xs mt-1">
                  {editingLink
                    ? "Cập nhật thông tin chi tiết cho liên kết của bạn."
                    : "Điền thông tin chi tiết cho liên kết tiểu sử mới."}
                </p>
              </div>
              <button
                onClick={() => {
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full bg-slate-50 border border-slate-200 rounded px-4 py-3 focus:ring-2 focus:ring-indigo-100 focus:border-indigo-500 outline-none transition-all text-sm text-slate-800"
                />
              </div>
                ...
                <div className="grid grid-cols-4 gap-2.5">
                  {PRESET_ICONS.map((preset) => (
                    <button
                      ...
                      className={`flex flex-col items-center justify-center p-2 rounded border transition-all aspect-square relative ${
              ...
              <div className="p-3.5 bg-slate-50 rounded-md border border-slate-150">
                ...
                {customIcon &&
                (customIcon.startsWith("/uploads/") ||
                  customIcon.startsWith("http://") ||
                  customIcon.startsWith("https://")) ? (
                  <div className="flex items-center gap-3">
                    <div className="w-12 h-12 bg-white rounded border border-slate-200 flex items-center justify-center p-1 shrink-0 shadow-sm">
                      <img
                        src={customIcon}
                        alt="Custom Logo"
                  ...
                  <div>
                    ...
                    <label
                      ...
                      className="flex items-center justify-center gap-2 border border-dashed border-slate-300 bg-white hover:bg-slate-50 rounded py-3 px-4 cursor-pointer text-xs font-semibold text-slate-600 transition-colors"
                    >
                      {isUploading ? (
                        <>
                    ...
                    <p className="text-[10px] text-slate-400 mt-1.5 font-mono leading-normal">
                      Hỗ trợ JPG, PNG, WEBP. Tự động cắt bo góc và vừa vặn kích
                      cỡ, không nền.
                    </p>
                  </div>
                )}
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full bg-slate-50 border border-slate-200 rounded px-4 py-3 pl-10 focus:ring-2 focus:ring-indigo-100 focus:border-indigo-500 outline-none transition-all text-sm text-slate-800"
                  />
              ...
              <div className="pt-2 flex gap-3">
                <button
                  ...
                  className="flex-1 bg-slate-50 text-slate-700 py-3.5 rounded font-bold hover:bg-slate-100 transition-colors text-sm"
                >
                  Hủy
                </button>
                <button
                  type="submit"
                  className="flex-1 text-white py-3.5 rounded font-bold hover:brightness-110 shadow-md hover:shadow-indigo-500/10 active:scale-95 transition-all text-sm"
                  style={{ backgroundColor: accentColor }}
                >
