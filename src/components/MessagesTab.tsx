...
export default function MessagesTab({
  ...
  const filteredMessages = messages.filter(msg => {
    ...
    return name.includes(term) || email.includes(term) || content.includes(term);
  });
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <div className="bg-white p-6 sm:p-8 rounded-2xl border border-slate-100 shadow-sm flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            <span className="p-2 rounded-lg bg-indigo-50" style={{ color: accentColor, backgroundColor: `${accentColor}10` }}>
              <LucideIcon name="MessageSquare" size={20} />
            </span>
            <h2 className="font-display text-xl font-bold text-slate-800">Tin nhắn từ độc giả</h2>
          </div>
          <p className="text-slate-400 text-xs pl-1">
            Nơi nhận các tin nhắn, phản hồi hoặc lời chúc trực tiếp từ những người truy cập trang hồ sơ của bạn.
          </p>
        </div>

        <div className="flex gap-4">
          <div className="bg-slate-50 border border-slate-100 p-3 px-4 rounded-xl text-center">
            <span className="block text-xl font-black text-slate-800">{messages.length}</span>
            <span className="text-[10px] uppercase font-mono tracking-wider text-slate-400 font-bold">Tổng số</span>
          </div>
          <div className="bg-indigo-50/40 border border-indigo-100/30 p-3 px-4 rounded-xl text-center" style={{ backgroundColor: `${accentColor}08` }}>
            <span className="block text-xl font-black" style={{ color: accentColor }}>
              {filteredMessages.length}
            </span>
            <span className="text-[10px] uppercase font-mono tracking-wider text-slate-400 font-bold">Tìm thấy</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {filteredMessages.map((msg) => (
          <div 
            key={msg.id} 
            className="bg-white border border-slate-100 hover:border-slate-200 rounded-2xl p-5 shadow-xs hover:shadow-md transition-all flex flex-col justify-between group"
          >
            <div className="space-y-3.5">
              ...
              <div className="flex justify-between items-start gap-2">
                <div className="space-y-0.5 min-w-0">
                  ...
                  <p className="text-xs font-mono text-slate-400 truncate">{msg.email || 'Không có email'}</p>
                </div>
              ...
              <div className="bg-slate-50/50 rounded-xl p-3 border border-slate-100 text-slate-700 text-xs leading-relaxed whitespace-pre-wrap">
            ...
            <div className="flex justify-between items-center pt-4 mt-2 border-t border-slate-50">
              <a 
                href={msg.email ? `mailto:${msg.email}?subject=Phản hồi từ Vivid Persona` : '#'} 
        ...
        {filteredMessages.length === 0 && (
          <div className="col-span-full bg-white rounded-2xl border border-dashed border-slate-200/80 p-16 text-center text-slate-400 flex flex-col items-center justify-center space-y-3">
            ...
            <div className="space-y-1">
              ...
              <p className="text-xs max-w-xs mx-auto">
                {searchTerm ? 'Không tìm thấy tin nhắn nào khớp với từ khóa tìm kiếm.' : 'Hộp thư hiện tại đang trống. Khi có người truy cập nhắn tin từ hồ sơ công khai, tin nhắn sẽ xuất hiện tại đây.'}
              </p>
            </div>
          </div>
        )}
