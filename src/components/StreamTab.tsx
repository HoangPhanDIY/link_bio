...
export default function StreamTab({
  ...
  const [testMessage, setTestMessage] = useState("Chúc bạn live stream vui vẻ và gặt hái nhiều thành công!");
  ...
  const DEFAULT_SOUNDS = [
    { name: "Chime cổ điển", url: "https://assets.mixkit.co/active_storage/sfx/2869/2869-84.wav" },
    { name: "Retro Level Up", url: "https://assets.mixkit.co/active_storage/sfx/2019/2019-84.wav" },
    { name: "Game Coin", url: "https://assets.mixkit.co/active_storage/sfx/2017/2017-84.wav" },
    { name: "Magic Wand", url: "https://assets.mixkit.co/active_storage/sfx/2018/2018-84.wav" },
  ];
  ...
  const handleGifUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (publicUrl) {
        ...
        window.showNotification?.("Tải lên GIF mới và lưu vào thư mục donate thành công!", "success");
      }
    } catch (err) {
      console.error(err);
    } finally {
  ...
  const handleSoundUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (publicUrl) {
        ...
        window.showNotification?.("Tải lên âm thanh mới và lưu vào thư mục donate thành công!", "success");
      }
    } catch (err) {
      console.error(err);
    } finally {
  ...
  const handleSendTestAlert = async (name: string, amount: number, message: string) => {
    setIsTesting(true);
    try {
      ...
      if (data.success) {
        ...
        if (appearance.streamAlertTts && "speechSynthesis" in window) {
          setTimeout(() => {
            let speakText = (appearance.streamAlertTemplate || "{name} đã ủng hộ bạn {amount}Đ")
              .replace("{name}", name)
            ...
            if (appearance.streamAlertVoiceName) {
              selectedVoice = voices.find((v) => v.name === appearance.streamAlertVoiceName);
            }
        ...
        window.showNotification?.("Đã gửi thông báo chạy thử lên OBS Stream Overlay!", "success");
      }
    } catch (err) {
      console.error(err);
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="space-y-1">
            ...
            <p className="text-sm text-slate-300">
              Công cụ hiển thị hộp quà tặng, phát âm thanh vui tai và đọc giọng nói (TTS) tự động trên livestream OBS mỗi khi có người ủng hộ (donate) thành công.
            </p>
          </div>
        ...
        <div className="mt-6 p-4 bg-slate-950/80 border border-slate-800 rounded-xl space-y-2">
          ...
          <div className="flex items-center gap-2">
            <input
              ...
              readOnly
              value={obsLink}
              className="flex-1 bg-slate-900/80 border border-slate-700/80 text-indigo-200 px-3 py-2 rounded-lg text-xs font-mono focus:outline-none"
            />
            <button
              onClick={copyOBSLink}
          ...
          <p className="text-[11px] text-slate-400 mt-1">
            * Thêm liên kết này vào OBS Studio dưới dạng <b>Browser Source</b> (Nguồn trình duyệt), cài đặt độ phân giải <b>1920x1080</b> và tích hợp âm thanh.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-7 space-y-6">
          <div className="bg-slate-900/40 border border-slate-800 rounded-2xl p-6 shadow-md space-y-6">
              ...
              <div className="flex gap-4 items-center">
                ...
                <div className="flex-1 space-y-2">
                  <input
                    ...
                    onChange={(e) => onUpdateAppearance({ streamAlertGif: e.target.value })}
                  ...
                  <label className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700 hover:border-slate-600 rounded-lg transition text-xs font-semibold cursor-pointer">
                    ...
                    <span>{isUploadingGif ? "Đang tải lên..." : "Tải lên tệp GIF"}</span>
                    <input
                      type="file"
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                ...
                <button
                  ...
                  className="text-xs text-indigo-400 hover:text-indigo-300 flex items-center gap-1"
                >
                  <History className="w-3 h-3" /> Sử dụng lại âm thanh cũ
                </button>
              </div>
              <div className="flex gap-3 items-center">
                <input
                  ...
                  onChange={(e) => onUpdateAppearance({ streamAlertSound: e.target.value })}
                ...
                <label className="inline-flex items-center gap-1.5 px-3 py-2 bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700 hover:border-slate-600 rounded-lg transition text-xs font-semibold cursor-pointer shrink-0">
                  <Upload className="w-3.5 h-3.5" />
                  <span>{isUploadingSound ? "Đang tải..." : "Tải lên âm thanh"}</span>
                  <input
                    type="file"
                    accept="audio/*"
                    className="hidden"
              ...
              {/* Quick default sound selector */}
              <div className="space-y-1.5">
                <div className="text-xs text-slate-400">Chọn nhanh âm thanh mặc định:</div>
                <div className="grid grid-cols-2 gap-2">
                  {DEFAULT_SOUNDS.map((snd, idx) => (
                    <button
                      key={idx}
                      onClick={() => {
                        onUpdateAppearance({ streamAlertSound: snd.url });
                        const a = new Audio(snd.url);
                        a.volume = 0.5;
                        a.play().catch(() => {});
                      }}
                      className={`px-3 py-1.5 text-left text-xs font-medium rounded-lg border transition flex items-center justify-between ${
            ...
            <div className="space-y-2">
              <div className="flex justify-between items-center">
                <label className="text-sm font-medium text-slate-200 block">
                  Cú pháp chữ thông báo (Template)
                </label>
              ...
              <input
                ...
                onChange={(e) => onUpdateAppearance({ streamAlertTemplate: e.target.value })}
                placeholder="Ví dụ: {name} đã ủng hộ bạn {amount}Đ"
                className="w-full bg-slate-950 border border-slate-800 text-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-indigo-500"
              />
              <p className="text-[11px] text-slate-400">
                * Đoạn chữ hiển thị trên màn hình OBS. Bạn có thể sửa thành: <i>{"{name}"} vừa tặng bạn {"{amount}"} VNĐ</i>
              </p>
            </div>
            ...
            <div className="space-y-4 p-4 bg-slate-950/50 border border-slate-800/80 rounded-xl">
              <h4 className="text-xs font-bold text-indigo-300 uppercase tracking-wider flex items-center gap-1.5">
                <Volume2 className="w-3.5 h-3.5" /> Cấu hình giọng đọc Google (TTS)
              </h4>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="flex items-center h-10 bg-slate-950 px-3 rounded-lg border border-slate-800">
                    <label className="flex items-center gap-2 cursor-pointer w-full select-none text-sm text-slate-300">
                      <input
                        ...
                        onChange={(e) => onUpdateAppearance({ streamAlertTts: e.target.checked })}
                ...
                <div className="space-y-2">
                  ...
                  <select
                    ...
                    onChange={(e) => onUpdateAppearance({ streamAlertVoiceGender: e.target.value })}
              ...
              {systemVoices.length > 0 && (
                <div className="space-y-2 pt-2 border-t border-slate-800/60">
                  ...
                  <select
                    ...
                    onChange={(e) => onUpdateAppearance({ streamAlertVoiceName: e.target.value })}
                  ...
                  >
                    <option value="">-- Tự động lọc theo giọng phía trên --</option>
                    {systemVoices.map((v, index) => (
                      <option key={index} value={v.name}>
                        {v.name} ({v.lang}) {v.localService ? "[Ngoại tuyến]" : "[Trực tuyến]"}
                      </option>
                    ))}
                  </select>
                </div>
            ...
            <div className="space-y-2">
              ...
              <input
                ...
                min={3}
                max={30}
                value={appearance.streamAlertDuration || 8}
                onChange={(e) => onUpdateAppearance({ streamAlertDuration: Number(e.target.value) })}
                className="w-full h-10 bg-slate-950 border border-slate-800 text-slate-200 rounded-lg px-3 text-sm focus:outline-none focus:border-indigo-500"
              />
            </div>
            ...
            <div className="pt-2">
              ...
              >
                {isSaving ? "Đang lưu cấu hình..." : "Lưu cấu hình và bảng (cau_hinh)"}
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-5 space-y-6">
          ...
          <div className="bg-slate-900/40 border border-slate-800 rounded-2xl p-6 shadow-md space-y-4">
            ...
            <div className="space-y-3">
              <div>
                <label className="text-xs text-slate-300 block mb-1 font-medium">Tên người donate test</label>
                <input
                  type="text"
                  value={testName}
              ...
              <div>
                <label className="text-xs text-slate-300 block mb-1 font-medium">Số tiền donate test (đ)</label>
                <input
                  type="number"
                  value={testAmount}
              ...
              <div>
                <label className="text-xs text-slate-300 block mb-1 font-medium">Lời nhắn test</label>
                <textarea
                  rows={2}
                  value={testMessage}
              ...
              <button
                onClick={() => handleSendTestAlert(testName, testAmount, testMessage)}
                disabled={isTesting}
              ...
              >
                ...
                <span>{isTesting ? "Đang gửi..." : "GỬI TEST LÊN STREAM (OBS)"}</span>
              </button>
            </div>
          </div>
          ...
          <div className="bg-slate-900 border border-slate-800 w-full max-w-2xl rounded-2xl overflow-hidden flex flex-col max-h-[85vh]">
            ...
            <div className="px-6 py-4 border-b border-slate-800 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-semibold text-white">Tái sử dụng tài nguyên đã tải lên</h3>
              </div>
              <button
                    ...
                    {historicalGifs.map((url, i) => (
                      <button
                        ...
                        onClick={() => {
                          ...
                          window.showNotification?.("Đã chọn sử dụng lại ảnh động!", "success");
                        }}
                        className={`group relative aspect-square bg-slate-950 border border-slate-800 rounded-xl overflow-hidden hover:border-indigo-500 transition-all ${
                          appearance.streamAlertGif === url ? "ring-2 ring-indigo-500" : ""
                        }`}
                      >
                        <img
                          src={url}
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <button
                              onClick={() => {
                                ...
                                setShowHistoryModal(false);
                                window.showNotification?.("Đã chọn sử dụng lại âm thanh!", "success");
                              }}
            ...
            <div className="px-6 py-4 border-t border-slate-800 bg-slate-950/60 flex justify-end gap-2 text-xs text-slate-400">
              * Toàn bộ tệp tin được lấy trực tiếp từ thư mục <b className="text-indigo-400 font-semibold">donate/</b> của bucket chứa ảnh của bạn.
            </div>
          </div>
        </div>
      )}
