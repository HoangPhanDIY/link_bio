  ...
  useEffect(() => {
    ...
    const playNotification = async () => {
      ...
      const safetyTimeout = setTimeout(() => {
        console.warn("Safety timeout triggered: alert was playing for too long, dismissing...");
        finishAlert();
      }, 35000);
      ...
      const playTTS = () => {
        return new Promise<void>((resolve) => {
          ...
          // Build TTS text
          const cleanTemplate = streamSettings.stream_alert_template || "đã ủng hộ bạn";
          let speakText = "";
          if (cleanTemplate.includes("{name}") || cleanTemplate.includes("{amount}")) {
            speakText = cleanTemplate
              .replace("{name}", nextAlert.name)
              .replace("{amount}", nextAlert.amount.toLocaleString("vi-VN"));
          } else {
            speakText = `${nextAlert.name} ${nextAlert.amount.toLocaleString("vi-VN")} đồng ${cleanTemplate}`;
          }
          ...
          if (useGoogleTTS) {
            ...
            try {
              ...
              ttsAudio.onerror = (e) => {
                console.warn("Google TTS failed/blocked, falling back to local speech synthesis:", e);
                clearTimeout(maxTtsTimer);
                isTtsPlaying = false;
              ...
              ttsAudio.play().catch((err) => {
                console.warn("Google TTS autoplay failed, falling back to local speech synthesis:", err);
                clearTimeout(maxTtsTimer);
                isTtsPlaying = false;
                playLocalSpeechSynthesis(speakText, resolve);
              });
      ...
      const playLocalSpeechSynthesis = (text: string, resolveFn: () => void) => {
        if (!("speechSynthesis" in window)) {
          resolveFn();
          return;
        }
      ...
      // Wait a very small delay (300ms) between sound and voice
